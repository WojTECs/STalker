...
std::vector<uint8_t> Interface::DownstreamData::MainConfigurationFrame::serialize()
{
    ...
    output.push_back(dataset.size());

    output.insert(output.end(), dataset.begin(), dataset.end());
    dataset.clear();
...
void Interface::DownstreamData::MainConfigurationFrame::deserialize(const uint16_t *msgArray, uint16_t arraySize)
{
//    try
//    {
